...
/* rotate a point */
static GLvoid rotPoint(GLfloat *p,GLfloat mat[3][3])
{
 GLfloat x,y,z;
...
/* glmCenter: Calculates the center (x, y, z) of
 * a model.
 *
 * model      - initialized GLMmodel structure
 * center - array of 3 GLfloats (GLfloat center[3])
 */
GLvoid
...
/* glmTranslate: Translate a model by a given vector.
 *
 * model - properly initialized GLMmodel structure
...
/* glmRotate: Rotate a model by a given angle and vector.
 *
 * model - properly initialized GLMmodel structure
 * x,y,z - components of the vector
 */
GLvoid glmRotate(GLMmodel *obj,GLfloat angle,GLfloat x,GLfloat y,GLfloat z)
{
 GLint i;
 GLfloat rotmat[3][3],len,sinth,costh;
 ...
 for (i=1; i<=obj->numvertices; i++)
	rotPoint(&obj->vertices[3*i],rotmat);
 for (i=1; i<=obj->numnormals; i++)
	rotPoint(&obj->normals[3*i],rotmat);
